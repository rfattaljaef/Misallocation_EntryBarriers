...
! we back-out implicit entry tax that would rationalize this average size, given the misallocation.
! then we compare it with the doing business one.
...
integer, parameter :: N=180, nty=180, Nco=21, NC=4, NR=24, NuE=1, NIN=6 !length of vector of Productivities, number of cases
...
real(8) :: Tau(N), Corr(N), TauFunc(N), TauEcase(NuE), eta, TauE, TauEH, TauEL, TauMax, TauMin !Distortions
...
!Calibration Targets
real(8) :: AvLE, AvLP,Lshare90, LshareTop, MshareTop, MshareBot
integer :: Index90
...
open (2,file="regcoeff_weighted_reg_tr5.txt")    !list of regcoeffs for countris for which I can do weighted least squares (for now, all except Malaysia), trimming tails at 5%
...
elseif (abs(ic-3)<0.001) then
   TauE=TauE  !! keep the TauE we just estimated
   TauEL=TauE
   TauEH=TauE
...
do itR=1,Nfp! Bisection on TauE to match average size given misallocation
...
ShareTax=1-cdfM(IndexZmedian+1)  !remember that the median firm gets tau=0
print *, 'Median', test6, itMed, Zmed, ZmedG

if (test6.le.0.00001) exit
ZmedG=Zmed
IndexZmedian=IndexM !minloc(abs(z-ZmedG),1)
...
MshareTop = sum(Mass(1:N), mask=EmpTot(1:N).ge.250)/sum(Mass)
MshareBot = sum(Mass(1:N), mask=EmpTot(1:N).lt.50)/sum(Mass)
...
! Turning on testTE only if we are in case looping on entry cost
if (abs(ic-2)>0.01) then
testTE=0.
else
testTE=AvSize10/AvSizeData(iy)-1 !/AvSizeFless-AvSizeTarget/AvSizeFless
endif

print *, 'EC', testTE, AvSize10, TauE, itR

if (testTE .lt. -0.03) then
	TauEL=TauE
	TauE=0.5*(TauEL+TauEH)
	else if (testTE .gt. 0.03)then
	TauEH=TauE
	TauE=0.5*(TauEL+TauEH)
endif

if (abs(testTE) .le. 0.03) exit
enddo  ! bisection on entry cost
...
Results(counter2,1:NR)=(/counter,eta,TauE,Me,Y,TFP,TFPpm,Za*Me/M,Lp,Wage,EntryR,ExitR,M,TauE*Wage*ne/(Y/L),AvSize,AvSize10,slopeL &
                      , LI, AvRDint,AvLE/AvLP, Lshare90, LshareTop,MshareTop,MshareBot/)
...
open(11,file="ResMasterAvs10indWeightedRegTr5.dat")
do i=1,NC*Nco
...
open(12,file="InnovMasterAvs10indWeightedRegTr5.dat")
do i=1,N
write(12,*) pCases(i,1:NC*Nco)
enddo
close(12)

open(13,file="LpMasterAvs10indWeightedRegTr5.dat")
do i=1,N
write(13,*) LpFirmCases(i,1:NC*Nco)
enddo
close(13)

open(14,file="EmpTotMasterAvs10indWeightedRegTr5.dat")
do i=1,N
write(14,*) EmpTotCases(i,1:NC*Nco)
enddo
close(14)

open(15,file="MassMasterAvs10indWeightedRegTr5.dat")
do i=1,N
write(15,*) MassCases(i,1:NC*Nco)
enddo
close(15)

open(15,file="ValMasterAvs10indWeightedRegTr5.dat")
do i=1,N
write(15,*) ValCases(i,1:NC*Nco)  !remember it's already multiplied by wage
enddo
close(15)
end program ssEndo
